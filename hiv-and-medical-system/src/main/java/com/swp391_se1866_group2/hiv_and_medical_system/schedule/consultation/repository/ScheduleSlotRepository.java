...
public interface ScheduleSlotRepository extends JpaRepository<ScheduleSlot, Integer> {
    ...
    @Query("SELECT sch FROM ScheduleSlot sch JOIN sch.slot sl WHERE sl.id = :slotId AND sch.status = 'UNAVAILABLE' ")
    List<ScheduleSlot> chooseDoctorBySlotId(@Param("slotId") int slotId, Pageable pageable);
